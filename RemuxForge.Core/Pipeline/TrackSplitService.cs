...
namespace RemuxForge.Core.Pipeline
{
    ...
    public class TrackSplitService
    {
        ...
        public TrackSplitService(string ffmpegPath, string tempFolder, string mkvMergePath = "")
        {
        ...
        public string ApplyEditMap(string langFile, int trackId, string trackType, string trackCodec, int channels, int sampleRate, EditMap editMap, string label)
        {
            ...
            List<TrackSegmentFile> segmentFiles;
            string concatFile;
            ...
            ConsoleHelper.Progress(LogSection.Deep, 93, "Deep: tracce");

            if (isSub)
            {
                SubtitleTimelineEditService subtitleService = new SubtitleTimelineEditService(this._ffmpegPath, this._tempFolder, this._ffmpegTimeoutMs, this._mkvMergePath);
                result = subtitleService.Apply(langFile, trackId, trackCodec, editMap, label);
                if (result.Length > 0)
                {
                ...
                {
                    ConsoleHelper.Write(LogSection.Deep, LogLevel.Error, "  Errore riscrittura timestamp sottotitoli traccia " + trackId);
                }
            ...
            {
                stretchedFile = this.ApplyStretch(langFile, trackId, trackCodec, channels, editMap.StretchFactor, label);
            ...
            segmentFiles = this.BuildAudioSegments(inputFile, trackId, trackCodec, channels, sampleRate, editMap, label);
            ...
            concatFile = this.ConcatSegments(segmentFiles, label, trackId);
            ...
            if (concatFile.Length > 0)
            {
                ...
                ConsoleHelper.Progress(LogSection.Deep, 98, "Deep: concatenazione");
            }
            else
            {
            ...
            {
                FileHelper.DeleteTempFile(segmentFiles[i].FilePath);
            }
        ...
        private string ApplyStretch(string langFile, int trackId, string trackCodec, int channels, string stretchFactor, string label)
        {
            string outputFile;
            ...
            codecArgs = this.GetCodecArgs(trackCodec, channels);
            ...
            ConsoleHelper.Progress(LogSection.Deep, 94, "Deep: stretch audio");
        ...
        /// <summary>
        /// Costruisce i segmenti audio (estratti + silenzi) dalle operazioni della EditMap
        /// </summary>
        ...
        private List<TrackSegmentFile> BuildAudioSegments(string inputFile, int trackId, string trackCodec, int channels, int sampleRate, EditMap editMap, string label)
        {
            List<TrackSegmentFile> files = new List<TrackSegmentFile>();
            ...
            for (int i = 0; i < editMap.Operations.Count; i++)
            {
                ...
                if (string.Equals(op.Type, EditOperation.INSERT_SILENCE, StringComparison.Ordinal))
                {
                    ...
                    {
                        segFile = this.ExtractAudioSegment(inputFile, trackId, currentStartMs / 1000.0, op.LangTimestampMs / 1000.0, label, segIdx);
                        if (segFile.Length > 0)
                        {
                            ...
                            this.AddSegment(files, segFile);
                            segIdx++;
                        }
                        else
                        {
                            this.CleanupSegments(files);
                            return new List<TrackSegmentFile>();
                        }
                    }

                    // Genera silenzio audio per compensare materiale presente nel source ma non nel lang
            ...
            segFile = this.ExtractAudioSegment(inputFile, trackId, currentStartMs / 1000.0, -1.0, label, segIdx);
            if (segFile.Length > 0)
            {
        ...
        /// <summary>
        /// Aggiunge un segmento temporaneo alla lista concat
        /// </summary>
        /// <param name="files">Lista segmenti</param>
        /// <param name="filePath">Path file segmento</param>
        private void AddSegment(List<TrackSegmentFile> files, string filePath)
        {
            TrackSegmentFile segment = new TrackSegmentFile();
            segment.FilePath = filePath;
            files.Add(segment);
        }

        /// <summary>
        /// Elimina segmenti temporanei gia' prodotti quando la costruzione fallisce
        /// </summary>
        /// <param name="files">Segmenti temporanei da eliminare</param>
        private void CleanupSegments(List<TrackSegmentFile> files)
        {
            for (int i = 0; i < files.Count; i++)
            {
                FileHelper.DeleteTempFile(files[i].FilePath);
            }
        }

        /// <summary>
        /// Estrae un segmento audio tramite ffmpeg stream copy
        /// </summary>
        ...
        private string ExtractAudioSegment(string inputFile, int trackId, double startSec, double endSec, string label, int segIndex)
        {
            string result = "";
            // Usa indice globale ffmpeg (0:N) perche' trackId e' l'ID mkvmerge globale
            string mapArg = "0:" + trackId;
            string outputFile = Path.Combine(this._tempFolder, label + "_seg" + segIndex + "_" + Guid.NewGuid().ToString("N").Substring(0, 8) + ".mka");
        ...
        private string GenerateSilence(string trackCodec, int channels, int sampleRate, int durationMs, string label, int segIndex)
        {
            ...
            string codecArgs = this.GetCodecArgs(trackCodec, channels);
        ...
        private string ConcatSegments(List<TrackSegmentFile> segmentFiles, string label, int trackId)
        {
            string result = "";
            string concatListFile = Path.Combine(this._tempFolder, label + "_concat_t" + trackId + "_" + Guid.NewGuid().ToString("N").Substring(0, 8) + ".txt");
            string outputFile = Path.Combine(this._tempFolder, label + "_deep_t" + trackId + "_" + Guid.NewGuid().ToString("N").Substring(0, 8) + ".mka");
            int exitCode;
            ...
            for (int i = 0; i < segmentFiles.Count; i++)
            {
                ...
                string escapedPath = segmentFiles[i].FilePath.Replace("\\", "/").Replace("'", "'\\''");
        ...
        private string GetCodecArgs(string trackCodec, int channels)
        {
            string result = "";
        ...
        /// <summary>
        /// Segmento temporaneo da passare al concat demuxer
        /// </summary>
        private class TrackSegmentFile
        {
            public TrackSegmentFile()
            {
                this.FilePath = "";
            }

            /// <summary>
            /// File segmento temporaneo
            /// </summary>
