...
namespace RemuxForge.Core.Audio
{
    ...
    public class AudioConversionService
    {
        ...
        public string ConvertTrack(string inputFile, int trackId, int channels, string label)
        {
            ...
            {
                extension = ".flac";
                codecArgs = "-c:a flac -compression_level " + AppSettingsService.Instance.Settings.Flac.CompressionLevel;
            }
            else if (string.Equals(this._format, "opus", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    codecArgs = "-af aformat=channel_layouts=" + channelLayout + " -c:a libopus -b:a " + bitrate + "k -mapping_family 1";
                }
                else
                {
                    codecArgs = "-c:a libopus -b:a " + bitrate + "k";
                }
            }
            ...
            outputFile = Path.Combine(this._tempFolder, label + "_t" + trackId + "_" + Guid.NewGuid().ToString("N").Substring(0, 8) + extension);

            ConsoleHelper.Write(LogSection.Conv, LogLevel.Notice, "  Conversione traccia " + trackId + " (" + channels + "ch) -> " + this._format.ToUpper());
            ConsoleHelper.Progress(LogSection.Conv, 45, "Conversione audio");
            ...
            if (exitCode == 0 && File.Exists(outputFile))
            {
                ...
                ConsoleHelper.Progress(LogSection.Conv, 90, "Conversione completata");
                result = outputFile;
            }
            else
            {
        ...
        private int RunFfmpeg(string inputFile, int trackId, string codecArgs, string outputFile, out string processOutput)
        {
            ...
            string[] baseArgs = new string[] { "-i", inputFile, "-map", "0:" + trackId };
